...
kernel void cellDerivate(device float4 * const delta_mean [[ buffer(0) ]],
	...
	const float4 jacob = exp(-0.5*(level_mean[n]*level_mean[n])/level_variance[n])/sqrt(2.0*M_PI*level_variance[n]);
	delta_mean[n] = jacob * state_error[n];
	delta_variance[n] = - 0.5 * jacob * level_mean[n] / level_variance[n];
}
