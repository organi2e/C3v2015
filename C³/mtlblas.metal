				  ...
				  threadgroup float4 * const accumulator [[ threadgroup(0) ]] )
{
	...
	if( !t )
		y[ rows ] = w.x * * accumulator + w.y * y[ rows ];
}
				 ...
				 )
{
	...
	threadgroup float4x4 * const aref = a + L * t.y;
	threadgroup float4x4 * const bref = b + L * t.x;
	...
	for ( uint i = 0, I = K ; i < I ; i += L ) {
		...
		aref[t.x] = rows_A < M && cols_A < K ? float4x4(A[cols_A*M+rows_A]) : float4x4(0.0);
		
		uint const rows_B = i + t.y;
		uint const cols_B = cols_C;

		bref[t.y] = rows_B < K && cols_B < N ? float4x4(B[cols_B*K+rows_B]) : float4x4(0.0);
...
kernel void fromRowMajorMatrix(device float4x4 * dst [[ buffer(0) ]],
							   ...
							   constant uint2 const & dim [[ buffer(2) ]],
							   uint2 const t [[ thread_position_in_grid ]],
							   uint2 const T [[ threads_per_grid ]]) {
	uint const m = t.y, M = dim.y;
	uint const n = t.x, N = dim.x;
